...
for word in prepositions:
	...
	cursor.execute('SELECT \"' + word + '\" FROM Cumulative_Overall WHERE \"year\" = ' + year)
	prev_overall = cursor.fetchone()[0]

	cursor.execute('SELECT \"' + word + '\" FROM Cumulative_Stranded WHERE \"year\" = ' + year)
	...
	cursor.executescript('UPDATE Cumulative_Overall SET \"' + word + '\" =' + num_overall + ' WHERE year=' + year + ';')

	cursor.executescript('UPDATE Cumulative_Stranded SET \"' + word + '\"=' + num_stranded + ' WHERE year=' + year + ';')

#Increment word table with number of words in the year
cursor.executescript('UPDATE Cumulative_Words SET Total=' + num_words + ' WHERE year=' + year)
cursor.executescript('UPDATE Cumulative_Overall SET Total=' + total_overall + ' WHERE year=' + year)
cursor.executescript('UPDATE Cumulative_Stranded SET Total=' + total_stranded + ' WHERE year=' + year)
